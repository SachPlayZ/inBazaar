...
export function parseQuantity(quantityStr: string): { value: number; unit: string } {
  const match = quantityStr.match(/^(\d+(?:\.\d+)?)\s*([a-zA-Z]+)$/);
  if (!match) {
    throw new Error("Invalid quantity format. Expected format: '5 kg' or '1.5 l'");
  }
  return {
...
export class BargainingAgent {
  ...
  private calculateStopLossPrice(basePrice: number, stopLossPercentage: number): number {
  ...
  private generateWeightedRandom(min: number, max: number, bias: number = 0.5): number {
    const r = Math.random();
  ...
  private calculateNextPrice(): number {
    ...
    if (progress < 0.3) {
      reductionRate = this.generateWeightedRandom(0.05, 0.10, 0.4);
    ...
    if (this.session.priceHistory.includes(newPrice)) {
      ...
      const adjustment = this.generateWeightedRandom(minAdjustment, maxAdjustment, 0.5);
      newPrice = Math.max(
        stopLossPrice,
        Math.min(
  ...
  private generatePriceSummary(status: 'pending' | 'accepted' | 'rejected'): PriceSummary {
    if (!this.session) {
      throw new Error("No active session");
    }
    ...
    return {
      ...
      totalReduction: Number((this.session.initialPrice - this.session.currentPrice).toFixed(2)),
      reductionPercentage: Number(((this.session.initialPrice - this.session.currentPrice) / this.session.initialPrice * 100).toFixed(2)),
  ...
  private async generateBargainingResponse(
    ...
    const priceReduction = previousPrice ? (previousPrice - currentPrice).toFixed(2) : "0.00";
    const percentageReduction = previousPrice ? ((previousPrice - currentPrice) / previousPrice * 100).toFixed(1) : "0.0";
...
Generate response:`;
  ...
  public async startBargaining(
    ...
    try {
      ...
      const initialReduction = this.generateWeightedRandom(minInitialReduction, maxInitialReduction, 0.4);
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          message,
          attemptCount: this.session.attemptCount,
          canContinue: !isAtStopLoss && this.session.attemptCount < this.MAX_ATTEMPTS,
